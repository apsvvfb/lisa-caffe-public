...
class videoRead(caffe.Layer):
  ...
  def setup(self, bottom, top):
    ...
    channel_mean = np.zeros((3,227,227)) #Three 227x227 matrices while all the elements are zero
    for channel_index, mean_val in enumerate(image_mean):
      channel_mean[channel_index, ...] = mean_val #all elements of 1st matrix become 103.939. all elements of 2rd matrix become 116.779. all elements of 3th...
  ...
  def join_worker(self):
    ...
    self.thread.join() # Wait until the thread terminates. This blocks the calling thread until the thread whose join() method is called terminates
    self.thread = None
